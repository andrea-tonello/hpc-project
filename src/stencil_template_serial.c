...
int main(int argc, char **argv)
{
	...
	// `planes` is an array of 2 pointers, each pointing to a `double` array. So:
	// - planes[OLD] points to the old grid
	// - planes[NEW] points to the new grid
	double *planes[2];
	...
	// If we're injecting less frequently than every iteration, do an initial injection.
	...
	*injection_frequency = *Niterations;
	...
	if ( freq == 0 )
		*injection_frequency = 1;
		...
		{
		freq = (freq > 1.0 ? 1.0 : freq );			// condition ? value_if_true : value_if_false
		*injection_frequency = freq * *Niterations;	// freq=0.1, Niterations=1000 --> every 100 iterations
		}
 ...
 */
{
	...
	// Total grid points including ghost cells:
	unsigned int bytes = (size[_x_]+2)*(size[_y_]+2);

	// malloc 2 * bytes, enough for both old and new planes
	planes_ptr[OLD] = (double*)malloc( 2*bytes*sizeof(double) );

	// Set memory to a value (here, 0). Initializes entire grid to zero.
	memset ( planes_ptr[OLD], 0, 2*bytes*sizeof(double) );

	// Pointer arithmetic: `planes_ptr[OLD]` points to start of memory block. 
	// Adding `bytes` moves the pointer forward by `bytes` doubles, 
	// so `planes_ptr[NEW]` points to the second half.
 ...
 */
{
	// Allocate array for source coordinates (2 ints per source: x and y)
	*Sources = (int*)malloc( Nsources * 2 *sizeof(uint) );

	// lrand48(): Generate random long integer.
	// `lrand48() % size[_x_]` gives random number from 0 to size[_x_]-1.
	// `1+`: Shift to 1 to size[_x_] (interior points, skipping ghost column 0).
...
int dump ( const double *data, const uint size[2], const char *filename, double *min, double *max )
{
	if ( (filename != NULL) && (filename[0] != '\0') )
		...
		*/
		...
		// `restrict`: promise this is the only pointer to this data (helps optimizer).
